...
void Modbus_RTU::setup(HardwareSerial* serial, uint8_t rx, uint8_t tx, uint8_t rts){
	#ifndef UNITTEST
	rs485comm.setup(serial, rx, tx, rts);
	#endif
}

void Modbus_RTU::begin(int8_t mode_, uint32_t baudrate_, uint32_t config_, uint8_t retries_){
	#ifndef UNITTEST
	rs485comm.begin( mode_, baudrate_, config_, retries_);
	#endif
}

void Modbus_RTU::change_config(int8_t mode_, uint32_t baudrate_, uint32_t config_, uint8_t retries_){
	#ifndef UNITTEST
...
bool Modbus_RTU::rs485_set_response(uint8_t* data, uint16_t size){
	if(size > 255)
		return false;
...
uint8_t Modbus_RTU::rs485_read(uint8_t unit_id, uint8_t fc, uint16_t address, uint16_t len, uint8_t* data, uint16_t* size){

	uint16_t buffer_size = *size;

  if(!Modbus_RTU::encode(unit_id,fc,address,len,(uint8_t*)nullptr,data, size)){
		#ifndef UNITTEST
		#ifdef WARNING_LOG
  ...
  if(Modbus_RTU::valid(data,len)){
    modbus_rtu rtu;
    #ifdef HIGH_DEBUG_LOG
    log("response valid");
    #endif
    if(Modbus_RTU::decode(data,len,&rtu)){
      #ifdef HIGH_DEBUG_LOG
    ...
    Serial.println("Received frame is not valid..");
		Serial.println(Modbus_RTU::getLastError());
		#endif
		#else
		std::cout << "Received frame is not valid.." << std::endl;
		std::cout << Modbus_RTU::getLastError() << std::endl;
		#endif
...
uint8_t Modbus_RTU::rs485_write(uint8_t unit_id, uint8_t fc, uint16_t address, uint16_t len, uint8_t* data, uint16_t* size){
  ...
  if(!Modbus_RTU::encode(unit_id,fc,address,len,data,frame, &payload_size)){
		#ifdef WARNING_LOG
	  log("error decoding");
	  #endif
		return ERROR_MODBUS_ENC;
  ...
  if(Modbus_RTU::valid(frame,*size)){
    modbus_rtu rtu;
    #ifdef HIGH_DEBUG_LOG
    log("response valid");
    #endif
    if(Modbus_RTU::decode(frame,payload_size,&rtu)){
      #ifdef HIGH_DEBUG_LOG
      log("rtu.slave_id: ",rtu.slave_id);
      log("rtu.fc: ",rtu.fc);
      log("rtu.len: ",rtu.len);
...
bool Modbus_RTU::valid(uint8_t *data, uint16_t len) {
	if (len < 4) return false;

	uint16_t crc = Modbus_RTU::crc(data, len - 2);

	return ( (((uint8_t) crc) == data[len - 2]) &&
	         (((uint8_t) (crc >> 8)) == data[len - 1]) );
}

bool Modbus_RTU::decode(uint8_t *data, uint16_t len, modbus_rtu* rtu) {
...
bool Modbus_RTU::encode(uint8_t slave_id, uint8_t fc, uint16_t address, uint16_t len, uint8_t* data, uint8_t *response, uint16_t* size) {
	...
	uint16_t crc = Modbus_RTU::crc(response, message_size);
...
uint16_t Modbus_RTU::encode(uint8_t *message, uint16_t message_size, uint8_t *response) {

	memcpy(message,response,message_size);

	uint16_t crc = Modbus_RTU::crc(message, message_size);
...
uint16_t Modbus_RTU::crc(uint8_t *buf, uint16_t len) {
	uint16_t crc = 0xFFFF;
...
String Modbus_RTU::getLastError(){
